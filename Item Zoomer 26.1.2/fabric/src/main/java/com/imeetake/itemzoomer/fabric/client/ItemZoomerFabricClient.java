    ...
    public void onInitializeClient() {
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
                ...
                ScreenKeyboardEvents.afterKeyRelease(screen).register((screen1, keyEvent) -> {
                    if (toggleKey.matches(keyEvent)) {
                        toggleKeyDown = false;
                    }
                });
            ...
            ScreenEvents.beforeExtract(screen).register((screen1, graphics, mouseX, mouseY, delta) -> {
                ZoomedItemRenderer.beginFrame();
            });

            ScreenEvents.afterExtract(screen).register((screen1, graphics, mouseX, mouseY, delta) -> {
