...
public class ZoomedItemRenderer {
    ...
    private static MultiBufferSource.BufferSource zoomBufferSource() {
        if (zoomBufferSource == null) {
            ...
            zoomBufferSource = MultiBufferSource.immediateWithBuffers(fixedBuffers, new BufferBuilder(786432));
        }
    ...
    private static void renderItemCount(GuiGraphics graphics, Minecraft mc, ItemStack stack, float x, float y, int size) {
        ...
        graphics.drawString(font, text, (int) textX, (int) textY, 0xFFFFFF, true);
    }
    ...
    private static void blitWithAlpha(RenderTarget source, int screenWidth, int screenHeight, float alpha) {
        ...
        buffer.vertex(0, 0, 0).uv(0, v).color(1f, 1f, 1f, alpha).endVertex();
        buffer.vertex(0, screenHeight, 0).uv(0, 0).color(1f, 1f, 1f, alpha).endVertex();
        buffer.vertex(screenWidth, screenHeight, 0).uv(u, 0).color(1f, 1f, 1f, alpha).endVertex();
        buffer.vertex(screenWidth, 0, 0).uv(u, v).color(1f, 1f, 1f, alpha).endVertex();
